...
| `queued — <role> is idle but wake failed (<reason>); …surface on its next prompt/register` | Idle but unreachable (no zellij / no tab / not zellij-registered / stale zellij session / no pane / write failed); no auto-redelivery until the recipient is next prompted or re-registers. |
