...
| **"A role keeps disappearing from `sessions/`."** | Session flapping — something fires `SessionEnd` → `radio unregister` on intra-session events (`/clear`, `/compact`, resume, or an unexplained cascade) and the wipe takes `TAB_ID` with it. #187 / #198 made `unregister` refuse unless a wipe is explicitly authorized. | Compare the three counters below. Post-#187, `skipping` should carry the bulk of the traffic and `unregister role=` should be close to `proceeding` plus however many `--manual` calls `task-done` made. If `role=` far exceeds `proceeding` + `skipping`, an **old `radio` binary** is running: every non-`--manual` call logs one or the other now. `radio` on `PATH` is a symlink into a checkout — run `ls -l "$(command -v radio)"` and confirm that tree is current. |
...
# Wipes vs. refusals (#187 / #198). `role=` counts every actual wipe,
# including the deliberate --manual ones task-done makes.
